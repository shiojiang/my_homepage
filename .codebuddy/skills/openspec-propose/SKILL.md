...
description: 创建新变更，一次性生成所有 artifact。当用户想快速描述要构建的内容并获得完整提案（包含设计、spec 和任务）时使用。
license: MIT
...
**输入**：用户的请求应包含变更名称（kebab-case）或想要构建的功能描述。

**步骤**

1. **如果没有提供明确输入，询问用户想构建什么**
...
3. **获取 artifact 构建顺序**
   ...
   - `planningHome`、`changeRoot`、`artifactPaths`、`actionContext`：路径和范围上下文。使用这些而非假设仓库本地路径。
