---
name: OPSX: 验证
description: "验证变更实现质量——编译检查、类型检查、Spec 验收"
argument-hint: "[命令参数]"
---

对已实现的变更进行全面验证。

**输入**：可选在 `/opsx:verify` 后指定变更名称（如 `/opsx:verify add-auth`）。如省略，尝试从对话上下文推断。
...
**问题：**
1. TypeScript 类型错误：`client/src/pages/HomePage.tsx:42` - 类型不匹配
